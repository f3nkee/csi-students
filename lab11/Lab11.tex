...
\begin{document}
	...
	\begin{gather}
	\displaystyle
	U\cdot \frac{K_u}{T_us+1}\cdot K_o - (x\cdot C_p +x\cdot s\cdot K_d) - F_{\text{в}} = m\cdot s^2\cdot x \\
	x\cdot(m\cdot s^2 +C_p + s\cdot K_d) =\frac{K_u\cdot K_o}{T_us+1}\cdot U
	\end{gather}
	Так как в данном случае входной величиной является $U$, а выходной - $x$, то передаточная функция будет равна:
	\begin{gather}
